...
import { fetchCurriculums } from '../../../features/curriculums/curriculumSlice';
...
const ManageCurriculum = () => {
  ...
  const { data: curriculums = [], loading, error } = useSelector((state) => state.curriculums);
  ...
  const filteredCurriculums = curriculums.filter((course) =>
	curriculum.lesson.toLowerCase().includes(searchQuery.toLowerCase()) 
  );
  ...
  const currentCourses = filteredCurriculums.slice(startIndex, endIndex);
  ...
  if (loading || curriculums.length === 6) {
	return (
	...
	<section className="bg-white p-4 my-10 rounded-lg shadow-md font-inter">
	  <div className="flex justify-between items-center mb-10 border-b border-tertiaryDark pb-3 -mx-6 px-6">
		<h2 className="flex gap-2 text-base font-bold">
		  <img src={icon} alt="dashboard icon" /> Curriculum List
		</h2>
		<div className="space-x-4 flex items-center">
	  ...
	  {totalItems === 0 ? (
		...
		<table className="w-[97%] mx-auto text-left border-l-2 border-t-2 border-collapse">
		  ...
		  <tbody>
			{currentCourses.map((curriculum, index) => (
			  <tr key={index} className="border-b border-tertiary font-normal text-xs">
				<td className="py-3 px-4 border-r-2 border-tertiary">
				  <div className="flex items-center gap-3">
				...
				<td className="p-2 border-r-2 border-tertiary">{curriculum.description}</td>
				<td className="p-2 border-r-2 border-tertiary">{curriculum.prize}</td>
				<td className="p-2 border-r-2 border-tertiary">{curriculum.startDate}</td>
				<td className="p-2 border-r-2 border-tertiary">{curriculum.endDate}</td>
				<td className="p-2 border-r-2 border-tertiary">{curriculum.action}</td>
			  </tr>
			))}
		  </tbody>
		</table>
