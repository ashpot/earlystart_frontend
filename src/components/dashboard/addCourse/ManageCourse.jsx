...
const ManageCourse = () => {
  ...
  const filteredCourses = courses.filter((course) =>
    ...
    course.category.title.toLowerCase().includes(searchQuery.toLowerCase())
  );
          ...
          <tbody>
            {currentCourses.map((course, index) => (
              <tr key={index} className="border-b border-tertiary font-normal text-xs">
                ...
                <td className="p-2 border-r-2 border-tertiary">{course.course.title}</td>
                <td className="p-2 border-r-2 border-tertiary">{course.description}</td>
  ...
  {course.video_url ? (
    ...
    >
      {course.video_url.length > 50
        ? `${course.video_url.substring(0, 50)}...`
        : course.video_url}
    </a>
  ) : (
    "-"
                ...
                <td className="p-2 border-r-2 border-tertiary">{course.duration}</td>
				<td className="p-2 border-r-2 border-tertiary">{course.action}</td>
              </tr>
            ))}
          </tbody>
        </table>
